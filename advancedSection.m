...
imageReference = imread('images/alleyReference.png');
 % standard deviation (different for each image!)
sigma = 20;
...
h = 0.55*sigma; %decay parameter
imageNoisy1 = double(imageNoisy);
imageReference = double(imageReference)/255;
tic;
%TODO - Implement the non-local means function
filtered = nonLocalMeans(imageNoisy1, sigma, h, patchSize, windowSize);
toc
...
[peakSNR, SNR] = psnr(imageNoisy1, imageReference);
